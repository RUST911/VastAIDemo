...
从 `dist/` 目录提供静态文件（SPA 回退），同时代理 Dify API 请求，端口 8081。
...
|------|------|--------|
| `PORT` | 后端服务端口 | `8081` |
| `LOCAL_IP` | 服务器 IP | `43.139.131.125` |
...
sudo systemctl enable --now vastbase-support
```
...
sudo systemctl stop vastbase-support
